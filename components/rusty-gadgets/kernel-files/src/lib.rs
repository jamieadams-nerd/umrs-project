...
fn parse_type_path_b(input: &str) -> io::Result<&str> {
    ...
    let _u = parts
        .next()
        .ok_or(io::Error::new(io::ErrorKind::InvalidData, "No User"))?;
    let _r = parts
        .next()
        .ok_or(io::Error::new(io::ErrorKind::InvalidData, "No Role"))?;
    let t = parts
        .next()
        .ok_or(io::Error::new(io::ErrorKind::InvalidData, "No Type"))?;
    Ok(t)
}
