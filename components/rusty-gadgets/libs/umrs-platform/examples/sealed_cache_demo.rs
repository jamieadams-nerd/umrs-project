...
fn main() {
    ...
    if !cache.caching_enabled() {
        println!("Note: FIPS mode is active — HMAC-SHA-256 (hmac + sha2) is not");
    ...
    println!(
        "  OS ID        : {}",
        result1.os_release.as_ref().map(|r| r.id.as_str()).unwrap_or("<not available>")
    );
    ...
    println!("  Key lifetime     : ephemeral (derived from boot_id + wall-clock nanos)");
    println!("  Key zeroized on  : SealedCache::drop (ZeroizeOnDrop)");
    println!("  Seal covers      : TrustLevel byte + SHA-256(EvidenceBundle) + identifiers");
    println!("  Seal failure     : discard, re-run pipeline, log::warn!");
    println!("  FIPS gate        : /proc/sys/crypto/fips_enabled (read at init, not at seal)");
    println!(
        "  TTL ceiling      : {} seconds",
    ...
    println!("    NIST SP 800-53 SC-28 — integrity protection for cached result");
