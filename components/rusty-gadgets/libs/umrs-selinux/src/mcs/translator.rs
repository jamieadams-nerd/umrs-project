...
///
/// The Global Singleton (auto-loaded on first access)
///
...
impl SecurityRange {
    ///
    /// Construct a degenerate range from a single MLS/MCS level.
    ///
...
/// Construct the path to the setrans.conf for the running policy, or `None`
/// if SELinux is not enabled or the policy type cannot be determined.
fn default_setrans_path() -> Option<String> {
    use crate::status::selinux_policy;
    let policy = selinux_policy()?;
    Some(format!("/etc/selinux/{}/setrans.conf", policy.as_str()))
}
...
fn load_into(translator: &mut Translator, path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Load complete. {} unique rules in memory.",
        translator.rules.len()
    );
    Ok(())
}
