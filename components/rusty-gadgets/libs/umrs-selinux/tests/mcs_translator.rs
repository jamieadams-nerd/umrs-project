...
fn reverse_lookup_known_marking_returns_ranges() {
    ...
    for (kernel_str, _detail) in results {
        SecurityRange::from_str(&kernel_str).unwrap_or_else(|e| {
            panic!(
...
fn category_order_normalization_should_not_change_lookup() {
    ...
    let ra = SecurityRange::from_str(a).expect("Failed to parse first range");
    let rb = SecurityRange::from_str(b).expect("Failed to parse second range");

    let la = guard.lookup(&ra).unwrap_or_else(|| "MISSING".to_string());
    let lb = guard.lookup(&rb).unwrap_or_else(|| "MISSING".to_string());
