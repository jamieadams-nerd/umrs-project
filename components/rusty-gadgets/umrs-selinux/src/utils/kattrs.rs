...
impl<'a, T: KernelFileSource + StaticSource> fmt::Display for AttributeCard<'a, T> 
where 
    T::Output: fmt::Debug 
{
...
impl KernelFileSource for SelinuxMls {
    ...
    const KERNEL_NOTE: &'static str = "Indicates if the Multi-Level Security (MLS) policy is active.";
...
impl KernelFileSource for SelinuxPolicyVers {
    ...
    const DESCRIPTION: &'static str = "The version of the SELinux policy language supported by the kernel.";
...
impl KernelFileSource for ProcFips {
    ...
    const KERNEL_NOTE: &'static str = "Verified via /proc/sys/crypto/fips_enabled (PROC_SUPER_MAGIC).";
...
impl KernelFileSource for GenericDualBool {
    ...
    const DESCRIPTION: &'static str = "Dynamic dual-boolean attribute (current pending)";
...
impl<T: KernelFileSource> SecureReader<T> {
    fn execute_read(&self, path: &Path, expected_magic: FsType) -> io::Result<T::Output> {
        ...
        let stats = statfs(path).map_err(|e| io::Error::other(e))?;
