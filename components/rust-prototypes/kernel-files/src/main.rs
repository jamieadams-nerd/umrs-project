use kernel_files::{
    AttributeCard, EnforceState, GenericDualBool, ProcFips, SecureReader,
    SelinuxEnforce, SelinuxMls, SelinuxPolicyVers, StaticSource,
...
fn main() {
    ...
    match enforce_reader.read() {
        Ok(EnforceState::Enforcing) => {
            println!("SELinux Enforcement:    Enforcing")
        }
        Ok(EnforceState::Permissive) => {
            println!("SELinux Enforcement:    Permissive")
        }
    ...
    match dual_reader.read_generic(&fips_mode_node) {
        Ok(state) => {
            println!(
                "SELinux fips_mode:     Current={}, Pending={}",
                state.current, state.pending
            );
        }
        Err(e) => {
            eprintln!(
                "SELinux fips_mode Err: {} (Boolean may not exist in current policy)",
                e
            );
        }
    }
    ...
    println!(
        "[Test B] Malformed (No trailing colon): {}",
        malformed_context
    );
