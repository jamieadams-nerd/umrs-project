# CPU Security Feature Matrix — Research Specification (v3)

**Version:** 3 (Phase 0.5 complete — ready for researcher consumption)
**Date:** 2026-03-17
**Supersedes:** v2 in `.claude/jamies_brain/archive/cpu-matrix.md`
...
**Phase 0.5 verification (2026-03-17):** All 10 Phase 0.5 actions confirmed incorporated:
1. Classification sub-tiers (Critical/Defensive vs Critical/Operational) — Section: Classification Definitions
2. 23-column matrix schema — Section: Matrix Schema
3. Categories 10 (Speculative Execution Mitigations, 8 features) and 11 (CPU-Enforced Access Controls, 7 features) — Section: Feature Categories + Complete Feature List
4. Mandatory research sections expanded to 13 items (CVE, compliance, virt confidence, disposition, fallback risk) — Section: Mandatory Research Sections
5. Authoritative sources expanded (NIST SP 800-90B, Intel CET, NSA RHEL, CMVP, CVE sources) — Section: Authoritative Source Pack
6. `/proc/crypto` as mandatory Layer 2 detection interface — Section: Linux Detection Interfaces
7. ARM/AArch64 equivalents (7 features in Category 15, cross-reference table) — Section: ARM/AArch64 Requirements
8. Feature interpretation framework Question 7 (compliance dimension) and Question 8 (unused = finding?) — Section: Feature Interpretation Rules
9. SMT as cross-cutting platform topology property (Feature 52) — Section: Complete Feature List, Category 14
10. Initial prioritization reordered: defensive features (Tier 1) before capability features (Tier 2-5) — Section: Initial Prioritization
