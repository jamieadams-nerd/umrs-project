...
The anchor `#_corepattern_tpi_classification_phase_2b` is auto-generated by Antora from
the section heading `== CorePattern — TPI Classification (Phase 2b)`.
Please verify this anchor resolves correctly in the next docs build review.
If the heading changes, the xref will silently break.
Consider whether this cross-page anchor should be replaced with a simpler xref or
whether the section heading should be stabilised.
