...
description: >
  High-assurance pattern rules for UMRS Rust development: must-use contracts,
  validate-at-construction, trust gates, security findings as data, compile-time
  path binding, fixed-size deterministic layout, signal evaluation paths, and
  debug log discipline. Use this skill when writing or reviewing Rust code that
  involves security-relevant types, #[must_use], Result constructors, kernel
  trust gates, SecurityObservation, CategorySet, evidence records, or debug
  logging of sensitive state. Trigger when the user or agent mentions
  high-assurance patterns, must_use, validate at construction, trust gate,
  findings as data, compile-time path, deterministic layout, signal evaluation,
  debug log discipline, or security-relevant type. Also trigger when creating
  new types that represent access control decisions, cryptographic material,
  audit events, or trust boundary crossings.
---
