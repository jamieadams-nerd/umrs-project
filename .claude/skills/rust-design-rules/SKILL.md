---
name: rust-design-rules
description: >
  Rust coding standards, clippy policy, citation format, system state read prohibition,
  annotation expectations, module documentation checklist, and performance-aware
  construction patterns for the UMRS project. Use this skill when writing or reviewing
  Rust code under components/rusty-gadgets/. Trigger when the user or agent mentions
  Rust code, clippy, cargo, forbid unsafe, citations, NIST annotations, module doc
  blocks, system state reads, std::fs, DIRECT-IO-EXCEPTION, OnceLock, splitn,
  must_use, or any Rust implementation work in the workspace. Also trigger when
  creating new .rs files, reviewing Rust PRs, or running cargo xtask commands.
---
