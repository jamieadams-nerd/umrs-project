...
- Helper extraction scripts retained in .claude/references/ (Termium and OQLF GDT parsers)
...
### CUI Labeling
- Knox extracted CUI banner marking rules from ISOO YouTube transcript into `.claude/references/cui-legal-corpus/cui-banner-marking-rules.md`
...
- **Guest Admin**: 4 HIGH / 8 MEDIUM / 6 LOW — 6.5/10 usability. Key: no operator docs, error messages use internal jargon, no --help/--json. Contradiction markers praised as strongest operator communication. Report: `.claude/reports/2026-03-20-tui-os-detect.md`
...
- **Team collaboration documentation**: Updated `.claude/agents/team-collaboration.md` with review routing rules, directory ownership, and auditor's tiered review format (ACCURATE/CONCERN/ERROR)
...
- **NIST SP 800-90B acquired**: PDF saved to `.claude/references/nist/sp800-90B.pdf` with manifest entry updated. Unblocks Phase 1B.
...
- **FedRAMP reference directory**: Created `.claude/references/fedramp/` with corrected URLs and 3 playbook/template documents
...
### Changed
- **FedRAMP reference URLs corrected**: Updated broken URLs in SOURCE.md and .claude/references/refs-manifest.md (Rev5 reorganization)
...
- **CMMC Final Rule (32 CFR Part 170)**: Downloaded 89 FR 83092 (Oct 15, 2024, effective Dec 16, 2024) to `.claude/references/dod-5200/cmmc-32cfr170-final-rule.pdf`; ingested into RAG collection `cmmc` (282 chunks)
- **CMMC Assessment Guide Level 2 v2.13**: Downloaded Sep 2024 edition from dodcio.defense.gov to `.claude/references/dod-5200/cmmc-assessment-guide-l2.pdf`; ingested into RAG collection `cmmc` (263 chunks)
- **NIST SP 800-171A Rev 3**: Downloaded assessment procedures companion to `.claude/references/nist/sp800-171Ar3.pdf`; ingested into `nist` RAG collection
- **DoD 5200.01 Information Security Program** (5 documents from esd.whs.mil): DoDI 5200.01, DoDM 5200.01 Volumes 1-3, DoDI 5200.48 (CUI); all downloaded to `.claude/references/dod-5200/`, ingested into RAG collection `dod-5200` (360 chunks)
...
- **Cross-team note**: Reference document topology guide — explains three-layer reference system (`rag-query` → `.claude/references/` → `.claude/references/`) for all agents
...
- `.claude/references/refs-manifest.md` updated to reflect current reference document inventory
