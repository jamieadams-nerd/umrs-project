...
MULTICS was developed by MIT, Bell Labs, General Electric. While MULTICS was not a full MLS system in the modern sense, it strongly influenced later secure OS research.
 However, MULTICS introduced ideas that would later become central to MLS, including:
...
Bell-LaPadula governs _confidentiality_: it prevents sensitive data from flowing to lower-sensitivity subjects.
It does not govern _integrity_ — it says nothing about preventing low-integrity data from flowing upward into a high-sensitivity context.
The complementary integrity model is the *Biba Model* (1977, Kenneth Biba), which inverts the Bell-LaPadula rules: a subject may not read data at a lower integrity level (no read down) and may not write data to a higher integrity level (no write up).
Bell-LaPadula and Biba together address both the confidentiality and integrity dimensions of information flow control.

Bell-LaPadula became the conceptual foundation for many MLS systems.
During this time, the U.S. Department of Defense introduced the Trusted Computer System Evaluation Criteria (TCSEC), commonly known as the Orange Book (1983).
TCSEC defined evaluation classes for secure systems, including requirements for MLS enforcement at higher assurance levels (B1, B2, B3, A1).
...
That combination is rare. Rather than “next-generation MLS operating systems,” research has focused on applying MLS principles in new places.
