...
The UMRS Rust tools are deliberately compiled without _vendoring_ cryptographic libraries, relying *instead* on the OpenSSL implementation provided and maintained by the operating system. In this context, “vendored OpenSSL” refers to a build model in which an application bundles and compiles its own private copy of the OpenSSL source code as part of the application build, rather than dynamically linking against the system-supplied OpenSSL libraries. 

[WARNING]
----
While vendoring can simplify portability across diverse platforms, it bypasses the operating system’s cryptographic subsystem and shifts responsibility for cryptographic maintenance, patching, and validation to the application itself.
----
...
So: using `OPENSSL_NO_VENDOR=1` plus the feature checks below gives you “I am correct” confidence.
...
2. Confirm you are dynamically linking to the system _libssl/libcrypto_ by running:
[bash,source]
----
...
3. Confirm the ELF “NEEDED” entries are for system libs by running:
[source,bash]
----
...
4. Look for a vendored build artifact (optional sanity check)

Vendoring typically leaves behind build output in target/…/build/openssl-sys-*/out/ that includes an OpenSSL source build tree.
...
4.	What I recommend you standardize in UMRS builds
To make this “always correct” and easy to document, standardize these in your build docs:
...
If you do those three checks, you can be confident and you can write a clean “verification appendix” statement like:
	•	“Builds are forced to use system OpenSSL (OPENSSL_NO_VENDOR=1). Feature graph is verified to exclude vendoring. Final ELF binary is verified to link against system libssl.so.3/libcrypto.so.3.”

If you want, I can rewrite the build section you’ll paste into UMRS docs to include exactly these verification commands and expected outputs, using your usual UMRS documentation tone.
