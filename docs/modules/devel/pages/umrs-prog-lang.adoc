...
Shell scripts are intentionally kept small, explicit, and defensive. They are not used for complex parsing, cryptography, or long-running logic. UMRS uses Bash where simplicity reduces risk, not where complexity would amplify it.
...
=== Recommended Secure Coding Reference

Developers writing Rust for UMRS should obtain and read the
https://anssi-fr.github.io/rust-guide/[Secure Rust Guidelines] published by ANSSI
(Agence nationale de la sécurité des systèmes d'information — France's national
cybersecurity agency). This is the recognized authoritative reference for writing
security-conscious Rust. It covers memory safety invariants, integer arithmetic
discipline, FFI safety boundaries, error handling patterns, and supply chain hygiene
